...
export default function TradeBox({
  ...
  const createComment = async (comment: string, signature: string, token: string) => {
    const created = await fetch(process.env.NEXT_PUBLIC_CLIENT_API_URL + "/replies", {
      method: "POST",
      body: JSON.stringify({
        text: comment,
        signature,
  ...
  const buy = async (comment?: string, token?: string) => {
    try {
      if (!amount) return;
      ...
      if (comment && token) createComment(comment, signature, token);
  ...
  const sell = async (comment?: string, token?: string) => {
    try {
      if (!amount) return;
      if (!bondingCurve) return;
      if (!pumpProgram) return;
  ...
  const handleTrade = async (comment?: string, _image?: File, token?: string) => {
    if (isBuySelected) {
      await buy(comment, token);
    } else {
      await sell(comment, token);
    }
  };
