...
export const EditProfile = ({
  ...
  useEffect(() => {
    ...
    console.log("clean", cleanImageUri);

    if (!usernameChanged)
      setUsername(user?.username || publicKey?.toBase58().slice(0, 6));
    if (!profileImageChanged) setProfileImage(cleanImageUri || "/pepe.png");
    if (!bioChanged) setBio(user?.bio || "");
  }, [user, publicKey]);
        ...
        <div className="grid gap-2">
          ...
          <div className="flex gap-2 w-fit justify-self-end">
            ...
            <button
              ...
              disabled={
                loading ||
                (!usernameChanged && !profileImageChanged && !bioChanged)
              }
              className={clsx(
