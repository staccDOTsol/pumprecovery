...
export default function Moderation() {
  ...
  const [unbanSuccessMessage, setUnbanSuccessMessage] = useState("");
  const toggleLogSelection = (event: any) => {
    event.preventDefault();
    setModerator(event.target.value)
  };
  const { logs } = useModeratorLogs({
    limit: LIMIT,
    offset,
    moderator
  });
  ...
  const unban = async () => {
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_CLIENT_API_URL}/moderation/ban/address/${unbanUser}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    if (res.ok) {
      setUnbanSuccessMessage("User unbanned");
    } else {
      setUnbanSuccessMessage("Error unbanning user");
    }
  };
  ...
  return (
    <div className="grid gap-2 p-4">
      <div className="flex gap-1 h-fit items-center text-white">
        ...
        <div
          onClick={() => {
            setSelectedTab("unban user");
            setOffset(0);
          }}
          className={clsx(
            "cursor-pointer px-1 rounded",
            selectedTab === "unban user" && "bg-green-300 text-black",
            selectedTab !== "unban user" && "hover:bg-gray-800 text-gray-500"
          )}
        >
          unban user
        </div>
        ...
        <div
          onClick={() => {
            setSelectedTab("moderator logs");
            setOffset(0);
          }}
          className={clsx(
            "cursor-pointer px-1 rounded",
            selectedTab === "moderator logs" && "bg-green-300 text-black",
            selectedTab !== "moderator logs" &&
      ...
      <div className="grid gap-4 text-white p-2">
        ...
        {selectedTab === "unban user" && (
          <div>
            <div className="w-fit flex gap-2 h-fit">
              <Input
                value={unbanUser}
                onChange={(e: any) => setUnbanUser(e.target.value)}
          ...
          <div className="grid gap-2">
            <div className="flex gap-4 mb-4">
              <Input
                ...
                onChange={toggleLogSelection}
                className="p-2 border border-gray-300 bg-white text-black"
              />
            </div>
          
              <div>{logs.map((v) => (
                <div className="p-1 border border-white" key={v.id}>
