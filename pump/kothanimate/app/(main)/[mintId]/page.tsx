...
export default function Trade({
  ...
  const { coin, fetchCoin, loading } = useCoin(mintId);
  const {
    getUSDMarketCap,
    bondingCurve,
    solPrice,
  ...
  if (!coin && !loading)
    return (
      <div className="text-center p-4 mt-4 border text-white">
        <p>
          Token not found. If you have recently created a token, please wait a
          few seconds for it to be indexed and then refresh.
        </p>
      </div>
    );
  if (loading)
    return (
      <div className="text-center p-4 mt-4 border text-white">Loading...</div>
    );
  if (!coin) return null;
              ...
              <div className="grid gap-1">
                {bondingCurve &&
                  ...
                  (coin.raydium_pool ? (
                    ...
                    <div className="p-4 w-fit bg-green-300 rounded mb-4">
                      bonding curve complete! a raydium pool will be seeded in
                      the next 5-20 minutes with $
                      {Math.floor(
                        lamportsToSol(getFinalVirtualSolReserves()) * solPrice
                      ).toLocaleString()}{" "}
                      of liquidity
