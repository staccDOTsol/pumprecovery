DROP FUNCTION IF EXISTS get_user_created_coins;
CREATE OR REPLACE FUNCTION get_user_created_coins(
    p_limit INTEGER,
    p_offset INTEGER,
...
BEGIN
    ...
    WHERE 
        ...
        AND (($3 IS NULL OR $3 = '''') OR 
            (c."name" ILIKE ''%'' || $3::TEXT || ''%'' OR 
             c.symbol ILIKE ''%'' || $3::TEXT || ''%'' OR 
             c.mint ILIKE ''%'' || $3::TEXT || ''%''))
        AND ($7 IS NULL OR c.creator = $7) -- Apply the filter for p_creator
    GROUP BY c.mint
    ORDER BY ' 
    || CASE 
