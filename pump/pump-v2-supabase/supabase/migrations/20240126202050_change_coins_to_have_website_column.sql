...
RETURNS TABLE (
    ...
    website text -- Added website column
) AS $$
BEGIN
  RETURN QUERY 
  SELECT c.mint, c.name, c.symbol, c."description", c.image_uri, c.metadata_uri, c.twitter, c.telegram, c.bonding_curve, c.associated_bonding_curve, c.creator, c.created_timestamp, c.raydium_pool, c.complete, m.twitter_username, u.pfp, c.virtual_sol_reserves, c.virtual_token_reserves, c.total_supply, c.website -- Added website to the SELECT
  FROM coins c
  LEFT JOIN (
    SELECT m1.*
    FROM messages m1
...
BEGIN
  ...
  SELECT 
    ...
    c."hidden",
    c.total_supply,
    c.website -- Added website to the SELECT
  FROM 
    coins c
  LEFT JOIN (
