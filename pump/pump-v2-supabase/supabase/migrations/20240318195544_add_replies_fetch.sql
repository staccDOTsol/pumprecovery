...
RETURNS TABLE (
    ...
    market_cap numeric,
    reply_count bigint, -- Added field for reply count
    last_reply bigint -- Added field for last reply timestamp
) AS $$
BEGIN
  RETURN QUERY 
  SELECT 
    ...
    c.market_cap,
    COUNT(r.mint) AS reply_count, -- Count of replies
    MAX(r.timestamp) AS last_reply -- Timestamp of the latest reply
  FROM 
    coins c
...
CREATE OR REPLACE FUNCTION get_coins(
    ...
    p_sort_key TEXT DEFAULT 'created_timestamp', -- Parameter for sorting key
...
RETURNS TABLE (
    ...
    king_of_the_hill_timestamp numeric,
    market_cap numeric,
    reply_count bigint -- Adding reply_count field
) AS $$
BEGIN
  RETURN QUERY 
  SELECT 
