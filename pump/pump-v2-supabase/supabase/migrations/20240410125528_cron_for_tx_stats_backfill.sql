CREATE TABLE IF NOT EXISTS public.update_tx_stats_results_backfill (
...
BEGIN
    ...
    FROM 
        tx_tracking tx
    WHERE 
        tx.signature NOT IN (SELECT signature FROM public.tx_stats_results) -- Ensuring uniqueness
    GROUP BY 
        tx.signature
    ON CONFLICT (signature) DO NOTHING; -- Skip if signature already exists
END;
$$ LANGUAGE plpgsql;
SELECT update_tx_stats_results_backfill();
