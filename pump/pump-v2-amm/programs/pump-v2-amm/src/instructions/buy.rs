...
pub mod buy {
    ...
    pub fn handler<'a>(ctx: anchor_lang::context::Context<Buy>, params: BuyParams) -> Result<()> {
        let base_amount = params.base_amount;
        let max_quote_amount = params.max_quote_amount;
        ...
        if ctx.accounts.referral_ata.key() != Pubkey::default() {
            ...
            spl_token_transfer(TokenTransferParams {
                ...
                destination: ctx.accounts.referral_ata.to_account_info(),
        ...
        emit!(BuyEvent {
            ...
            user: *ctx.accounts.user.to_account_info().key,
            timestamp: Clock::get()?.unix_timestamp as u64,
            referrer: *ctx.accounts.referral_ata.to_account_info().key,
            referrer_fee_amount,
            protocol_fee_amount
        });
