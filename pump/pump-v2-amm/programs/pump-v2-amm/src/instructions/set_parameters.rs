...
pub mod set_parameters {
    ...
    pub fn handler(ctx: Context<SetParameters>, params: SetParametersParams) -> Result<()> {
        let protocol_fee_bps = params.protocol_fee_bps;
        let referrer_fee_bps = params.referrer_fee_bps;
        let referrer_fee_discount_bps = params.referrer_fee_discount_bps;
        require!(
